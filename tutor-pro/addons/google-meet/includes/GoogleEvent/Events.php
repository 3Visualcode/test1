...
class Events {
	...
	public function create_meeting() {
		...
		$child_exists=isset($post['course_dup_id']) && !empty($post['course_dup_id']);
		...
		if ( count( $validation_error ) ) {
			...
			try {
				...
				if ( $is_update ) {
					...
					if($child_exists){
						$event_data_child=$event_data;
						$event_data_child['post_parent']=$post['course_dup_id'];
						$insert_event_child = EventsModel::insert( $event_data_child );
					}
