...
class EmailNotification {
	...
	public function tutor_announcements_notify_students( $announcement_id, $announcement, $action_type = '' ) {
		...
		foreach ( $enrolled_students as $enrolled_student ) {
			...
			// Get user mobile phone number in order to send the sms
			$to_user_mobile[] = strval(get_user_meta($enrolled_student->ID, "mobile", true));
		...
		/*
		 * New announcement on informatica wordpress course
		 *
		 * ## titolo annuncio - hello world ##
		 *
		 * ## sommario - hello world ##
		 *
		*/

		/* 
			When a new announcement is ready, notify the user also with a sms
		*/
		if ( get_option('sms_manager_checkbox_announcement_state') === '1' ) {
			/*
			 * Send the sms to the students of this class
			*/
			$sms_max_length = 155;
			$append_text = "...Continua sulla mail";
			if ( mb_strlen($sms) > $sms_max_length ) {
				// Truncate the string at the maximum length allowed before appending text
				$truncated = mb_substr($sms, 0, $sms_max_length - mb_strlen($append_text));

				// Check for the last occurrence of a space or new line characters
				$last_space = mb_strrpos($truncated, ' ');
				$last_new_line = max(mb_strrpos($truncated, "\r"), mb_strrpos($truncated, "\n"));

				// Choose the latest position from space or new line to cut the string
				if ($last_space !== false || $last_new_line !== false) {
					$last_cut_position = max($last_space, $last_new_line);
					$truncated = mb_substr($truncated, 0, $last_cut_position);
				}

				// Trim any whitespace or new line characters left at the end of the string
				$truncated = trim($truncated);

				// Append the text and return
				$final_sms = $truncated . $append_text;
				//echo "Sono qua";
				//print_r($to_user_mobile);
				wp_sms_send( $to_user_mobile, $final_sms, $is_flash );
			} 
			else {
				//echo "Sono qua 2";
				//print_r($to_user_mobile);
				wp_sms_send( $to_user_mobile, $sms, $is_flash );
			}
		}
	...
	private function enqueue_email( $to, $subject, $message, $headers, $attachments = array(), $force_enqueue = false, $batch = false ) {
		...
		foreach ( $to as $email ) {
			...
			if( ! isset( $insert_data['headers'] ) ) $insert_data['headers'] = '';
			if( isset( $insert_data['batch'] ) ) unset( $insert_data['batch'] );
