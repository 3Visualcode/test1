...
class Utils {
	...
	public function get_students( $start = 0, $limit = 10, $search_term = '', $course_id = '', $date = '', $order = 'DESC' ) {
		...
		if(isset($_GET['marco_test'])){
			echo "SELECT user.* FROM {$wpdb->posts} AS posts
				...
				WHERE posts.post_type = 'tutor_enrolled'
					...
					{$course_query}
					{$date_query}
					AND (user.display_name LIKE $search_term OR user.user_email = $search_term_raw OR user.user_login LIKE $search_term)
