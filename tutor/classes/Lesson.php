	...
	public function save_lesson_meta( $post_ID ) {
		...
		if ( '-1' === $video_source ) {
			...
			if($video_source === 'embedded'){
				//<iframe src="https://drive.google.com/file/d/1xNge_Fqd0rdkaC4Q7RwEtkkXIbCn2T9R/preview"></iframe>
				$url_array = explode("/", $video["source_embedded"]);
				$last_index = count($url_array)-1;
				$url_array[$last_index] = "preview";

				$url = implode("/", $url_array);
				$video["source_embedded"] = "<iframe src='$url'></iframe>";
			}
